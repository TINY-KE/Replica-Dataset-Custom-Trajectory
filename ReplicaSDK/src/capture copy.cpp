...
int main(int argc, char* argv[]) {
  ...
  while (!pangolin::ShouldQuit()) {
    ...
    if (meshView.IsShown()) {
      ...
      // 若用户点击了 Capture 按钮，则执行：
      if (pangolin::Pushed(captureBtn)){
        // 获取当前相机的 World-to-Camera 矩阵， 保存至 camPose.txt
        ...
        // save image
        char filename[1000];
        snprintf(filename, 1000, "%s/rgb/frame%06d.png", outFolder.c_str(), frameCnt);
        pangolin::SaveImage(
        ...
        if (renderDepth) {
          ...
          snprintf(filename, 1000, "%s/depth/depth%06d.png", outFolder.c_str(), frameCnt);
          pangolin::SaveImage(
